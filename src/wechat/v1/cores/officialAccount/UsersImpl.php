...
use common\lib\wechat\v1\Exception\officialAccount\UserException;
use common\lib\wechat\v1\services\officialAccount\Users;
...
class UsersImpl implements Users
{
	...
	public static function remark($openId, $remark, $app)
	{
		try{
			/**@var  $app \EasyWeChat\OfficialAccount\Application **/
			$user = $app->user->remark($openId,$remark);
			return $user;
		}catch (\Throwable $exception){
			throw new UserException($exception->getMessage());
		}
	...
	public static function blacklist($beginOpenid = null, $app)
	{
		try{
			/**@var  $app \EasyWeChat\OfficialAccount\Application **/
			$user = $app->user->blacklist($beginOpenid);
			return $user;
		}catch (\Throwable $exception){
			throw new UserException($exception->getMessage());
		}
	...
	public static function changeOpenId($oldAppId, $openidList)
	{
		try{
			/**@var  $app \EasyWeChat\OfficialAccount\Application **/
			$user = $app->user->changeOpenid($oldAppId,$openidList);
			return $user;
		}catch (\Throwable $exception){
			throw new UserException($exception->getMessage());
		}
